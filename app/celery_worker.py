...
# --- שלב 3: חילוץ נתוני השמע הגולמיים (PCM) ---
        pcm_data = response.candidates[0].content.parts[0].inline_data.data

        if not pcm_data:
            raise ValueError("No audio data was generated by the API.")

        # --- שלב 4: בניית קובץ WAV תקין בזיכרון ---
        # השתמשנו בערכים מהדוגמה של גוגל: ערוץ 1 (מונו), קצב דגימה 24kHz
        buffer = io.BytesIO()
        ...
        # קבלת כל המידע מהבאפר (כולל ה-headers שנוצרו)
        wav_data = buffer.getvalue()

        # --- שלב 5: שליחת קובץ ה-WAV השלם ל-Webhook ---
        file_name = f"{phone_number}.wav"
        # אנו שולחים את wav_data, לא את ה-pcm_data המקורי
